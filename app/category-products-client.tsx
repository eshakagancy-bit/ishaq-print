...
export default function CategoryProductsClient({ category, products }: { category: PublicEnabledCategory; products: StoredProduct[] }) {
  ...
  const visibleProducts = products.filter((product) => `${product.name} ${product.family} ${product.description}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return <main className="category-products-page" dir="rtl">
    ...
    <section className="container category-products-content">
      ...
      })}</div> : <div className="empty-state"><b>{query ? "لا توجد نتائج مطابقة" : `لا توجد منتجات في ${categoryLabels[category]} حاليًا`}</b><p>{query ? "جرّب البحث باسم آخر." : "يمكنك العودة لاحقًا أو التواصل معنا لمعرفة المتوفر."}</p></div>}
    </section>
