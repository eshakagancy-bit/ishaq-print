...
const categoryLabels: Record<PublicEnabledCategory, string> = { printers: "جميع الطابعات", inks: "جميع الأحبار", papers: "جميع الأوراق", laser_inks: "جميع أحبار الليزر" };

function productSlug(product: StoredProduct) {
  if (isInkCategory(product.category)) return getInkSlug(product);
...
export default function CategoryProductsClient({ category, products, allProducts, settings }: { category: PublicEnabledCategory; products: StoredProduct[]; allProducts: StoredProduct[]; settings: SiteSettings }) {
  ...
  const visibleProducts = useMemo(() => {
    ...
    const modelMatches = products.filter((product) => (product.models ?? []).filter((model) => model.isActive).flatMap((model) => [model.model, model.partNumber, model.compatibility, ...(model.variants ?? []).flatMap((variant) => [variant.partNumber, variant.color])]).filter(Boolean).join(" ").toLocaleLowerCase("ar").includes(normalizedQuery));
    return [...new Map([...productMatches, ...modelMatches].map((product) => [product.id, product])).values()];
  }, [products, query]);
  const selectedRows = selected
    ? isInkCategory(selected.category) ? buildInkSpecificationRows(selected) : selected.category === "papers" ? buildPaperSpecificationRows(selected) : buildQuickViewSpecificationRows(selected)
    : [];
  ...
  return <main id="main-content" tabIndex={-1} className="category-products-page" data-category={category} dir="rtl">
    ...
    <section className="container category-products-content">
      ...
      {visibleProducts.length ? <div className="category-products-list">{visibleProducts.map((product) => {
        ...
        const detailsHref = `/${isInkCategory(product.category) ? "inks" : category}/${productSlug(product)}`;
        const cartCategory = product.category === "printers" ? "printers" : product.category === "papers" ? "papers" : "inks";
        return <article className="category-product-row" key={product.id} role="link" tabIndex={0} aria-label={`فتح تفاصيل ${displayName(product)}`} onClick={(event) => { if (!(event.target as HTMLElement).closest("button,a")) router.push(detailsHref); }} onKeyDown={(event) => { if (event.target === event.currentTarget && event.key === "Enter") router.push(detailsHref); }}>
          <div className="category-product-image">{category !== "printers" && product.badge?.trim() && <span className="product-badge">{product.badge}</span>}<ProductCardCartButton category={cartCategory} productId={String(product.id)} productName={displayName(product)} productUrl={detailsHref} image={product.image || "/brand/eshak-logo.png"} inkVariantCount={product.inkSpecifications?.variants.length} modelCount={(product.models ?? []).filter((model) => model.isActive).length}/><button type="button" className={favorites.includes(product.id) ? "heart active" : "heart"} onClick={() => toggleFavorite(product.id)} aria-label={favorites.includes(product.id) ? "إزالة من المفضلة" : "إضافة إلى المفضلة"} aria-pressed={favorites.includes(product.id)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20.8 4.6a5.4 5.4 0 0 0-7.6 0L12 5.8l-1.2-1.2a5.4 5.4 0 0 0-7.6 7.6L12 21l8.8-8.8a5.4 5.4 0 0 0 0-7.6Z" /></svg></button>{isInkCategory(product.category) ? <InkImageCarousel images={images} alt={displayName(product)} /> : <Link className="product-image-link" href={detailsHref}><Image src={product.image || "/brand/eshak-logo.png"} alt={displayName(product)} width={420} height={320} sizes="(max-width: 700px) 46vw, (max-width: 1100px) 30vw, 280px" /></Link>}<button type="button" className="quick-view" onClick={(event) => openQuickView(product, event.currentTarget)} aria-label={`تفاصيل سريعة لـ ${displayName(product)}`}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.6"/></svg><span>تفاصيل سريعة</span></button></div>
          <div className="category-product-content">{product.family && <span className="product-family">{product.family}</span>}<h2><Link href={detailsHref}>{displayName(product)}</Link></h2>{category === "papers" && getPaperAvailabilityLabel(product) && <span className="product-availability" data-availability>{getPaperAvailabilityLabel(product)}</span>}<ProductModelChips product={product} detailsHref={detailsHref} />{!isLaserInkCategory(product.category) && <div className="category-product-price"><small>السعر</small><strong>{productPriceLabel(product.price)}</strong></div>}<div className="category-product-actions"><Link href={detailsHref}>لمعرفة المزيد</Link></div></div>
        </article>;
