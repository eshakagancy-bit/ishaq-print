    ...
    <section className="container category-products-content">
      ...
      {visibleProducts.length ? <div className="category-products-list">{visibleProducts.map((product) => {
        ...
        return <article className="category-product-row" key={product.id} onClick={(event) => { if (!(event.target as HTMLElement).closest("button,a")) openQuickView(product, event.currentTarget); }}>
          <div className="category-product-image"><button type="button" className={favorites.includes(product.id) ? "heart active" : "heart"} onClick={() => toggleFavorite(product.id)} aria-label={favorites.includes(product.id) ? "إزالة من المفضلة" : "إضافة إلى المفضلة"}>♥</button>{product.category === "inks" ? <InkImageCarousel images={images} alt={displayName(product)} /> : <button type="button" className="product-image-trigger" onClick={(event) => openQuickView(product, event.currentTarget)} aria-label={`عرض التفاصيل السريعة لـ ${displayName(product)}`}><Image src={product.image || "/brand/eshak-logo.png"} alt={displayName(product)} width={420} height={320} sizes="(max-width: 700px) 92vw, 280px" /></button>}</div>
          <div className="category-product-content">{product.family && <span className="product-family">{product.family}</span>}<h2>{displayName(product)}</h2>{product.description && <p>{product.description}</p>}<button type="button" className="quick-view" onClick={(event) => openQuickView(product, event.currentTarget)}>تفاصيل سريعة</button><div className="category-product-actions"><a href={whatsappLink(product)} target="_blank" rel="noreferrer">اطلب من المختص</a></div></div>
        </article>;
