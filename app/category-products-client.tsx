    ...
    <section className="container category-products-content">
      ...
      {visibleProducts.length ? <div className="category-products-list">{visibleProducts.map((product) => {
        ...
        return <article className="category-product-row" key={product.id} role="link" tabIndex={0} aria-label={`فتح تفاصيل ${displayName(product)}`} onClick={(event) => { if (!(event.target as HTMLElement).closest("button,a")) router.push(detailsHref); }} onKeyDown={(event) => { if (event.target === event.currentTarget && event.key === "Enter") router.push(detailsHref); }}>
          ...
          <div className="category-product-content">{category === "printers" ? <div className="printer-text-panel"><h2><Link href={detailsHref}>{displayName(product)}</Link></h2>{product.family && <span className="product-family" dir="auto">{product.family}</span>}</div> : <>{product.family && <span className="product-family">{product.family}</span>}<h2><Link href={detailsHref}>{displayName(product)}</Link></h2></>}{category === "papers" && getPaperAvailabilityLabel(product) && <span className="product-availability" data-availability>{getPaperAvailabilityLabel(product)}</span>}<div className="category-product-price"><small>السعر</small><strong>{productPriceLabel(product.price)}</strong></div><div className="category-product-actions"><Link href={detailsHref}>لمعرفة المزيد</Link></div></div>
        </article>;
