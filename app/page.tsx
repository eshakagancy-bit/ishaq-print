...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/api/hero-slides").then(async (response) => {
  ...
  }, [heroPaused, heroSettings.autoplayDelay, heroSettings.autoplayEnabled, heroSlides.length]);
  ...
  const changeHeroSlide = (step: number) => {
    ...
    setActiveHeroSlide((current) => (current + step + heroSlides.length) % heroSlides.length);
  };
