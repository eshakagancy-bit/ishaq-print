...
test("homepage follows the image-led storefront journey", async () => {
  ...
  assert.match(categoryLinks, /STOREFRONT_CATEGORY_ORDER: PublicEnabledCategory\[\] = \["printers", "papers", "inks", "laser_inks"\]/);
...
test("homepage product tiles use factual category lines instead of price and commercial footer", async () => {
  ...
  assert.match(card, /<div className="homepage-product-text-panel"><h3>/);
  ...
  assert.match(styles, /\.homepage-product-text-panel \{[^}]*border-radius:7px;[^}]*background:#0ca1c1;[^}]*color:#fff;/);
});

test("homepage categories are four unified square-artwork links without legacy card details", async () => {
  const [home, styles, categoryLinks] = await Promise.all([read("app/home-client.tsx"), read("app/globals.css"), read("app/storefront-category-links.tsx")]);
  ...
  assert.match(section, /<StorefrontCategoryLinks products=\{products\}\/>/);
  assert.equal((categoryLinks.match(/className="storefront-category-card"/g) ?? []).length, 1, "one mapped category-link template renders the four current categories");
  ...
  assert.match(categoryLinks, /isLaserInkCategory\(product\.category\)/);
  assert.match(styles, /\.storefront-category-image \{ width:clamp\(190px,20vw,270px\);/);
  assert.match(styles, /@media \(max-width:1100px\)[\s\S]*?\.storefront-category-image \{ width:clamp\(160px,19vw,210px\); \}/);
  assert.match(categoryLinks, /sizes="\(max-width: 760px\) 42vw, \(max-width: 1100px\) clamp\(160px, 19vw, 210px\), clamp\(190px, 20vw, 270px\)"/);
  ...
  assert.match(styles, /@media \(max-width:760px\)[\s\S]*?\.storefront-category-grid \{ grid-template-columns:repeat\(2,minmax\(0,1fr\)\); gap:24px 12px; \}/);
