...
test("drawer supports every close path and modal focus behavior", () => {
  assert.match(home, /className="drawer-close"[^>]*onClick=\{\(\) => setMenuOpen\(false\)\}/);
  assert.match(home, /if \(event\.target === event\.currentTarget\) setMenuOpen\(false\)/);
  assert.match(home, /event\.key === "Escape"/);
  assert.match(home, /setMenuOpen\(false\);[\s\S]*?requestSectionScroll/);
