...
test("home uses a wide desktop container with five-by-two product groups without changing mobile", async () => {
  ...
  assert.match(styles, /\.product-group \{[^}]*grid-template-columns:repeat\(5,minmax\(0,1fr\)\);[^}]*grid-template-rows:repeat\(2,auto\);[^}]*gap:20px;/);
